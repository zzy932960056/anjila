...
<body class="page-header-fixed">
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							...
							<div class="portlet-title">
								...
								<div class="caption"><i class="icon-edit"></i>园所分部列表</div>
							...
							<div class="portlet-body">
								...
								<table class="table table-striped table-hover table-bordered" id="sample_editable_1">
									...
									<tbody>
										@foreach($kinder as $ki)
										<tr class="">

											<td>{{$ki->id}}</td>

											<td><img src="{{$ki->kinder_cover}}" width="255"/></td>

											<td>{{$ki->kinder_name}}</td>

											<td>{{$ki->kinder_address}}</td>

											<td>{{$ki->kinder_tel1}}</td>

											<td>{{$ki->kinder_tel2}}</td>

											<td>{{$ki->is_first == 1 ? '是' : '否'}}</td>

											<td><img src="{{$ki->first_picture}}" width="255"/></td>

											<td><a href="/admin/kindergarten/label/{{$ki->id}}">园所详情</a></td>

											<td><a href="/admin/kindergarten/update/{{$ki->id}}">编辑</a></td>
	...
	<script>
		...
		$("#sample_editable_1_new").click(
			function(){
				location.href = "/admin/kindergarten/insert";
			}
		)
		...
		$(".delete").click(
				function(othis){
					...
					$.ajax({
						url:"/admin/kindergarten/dodelete",
						type:"post",
						data:{'mid':data},
						success:function(content){
							if(content.msg == 'ok'){
								alert('删除园所成功');
								location.href = "/admin/kindergarten";
